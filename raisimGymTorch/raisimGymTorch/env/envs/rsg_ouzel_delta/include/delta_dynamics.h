...
static constexpr double kDefaultPosIntegratorMax =
    0.2; // [m] compensate for 20cm offset
static constexpr double kDefaultQIntegratorMax =
    0.2; // [rad] compensate for 0.2 rad offset
...
const std::vector<std::uint_fast8_t> upper_region = {0b00000100, 0b00000000,
                                                     0b00000011};
const std::vector<std::uint_fast8_t> lower_region = {0b00000010, 0b00000101,
                                                     0b00000000};
...
public:
  ...
  void initialize(const double &r_B, const double &r_T, const double &l_P,
  ...
  void updateIntegrators(const Eigen::Vector3d &pos_error,
  ...
  void updateState(const Eigen::Quaterniond &base_q_WB,
                   const Eigen::Vector3d &base_vel_B,
                   const Eigen::Vector3d &base_angvel_B,
                   const Eigen::VectorXd &q_in, const Eigen::VectorXd &qd_in,
                   const Eigen::VectorXd &effort_in, const double &dt);

  void updateState(const Eigen::Quaterniond &base_q_WB,
                   const Eigen::Vector3d &base_vel_B,
                   const Eigen::Vector3d &base_angvel_B,
                   const Eigen::Vector3d &q_in, const Eigen::Vector3d &qd_in,
                   const Eigen::Vector3d &effort_in, const double &dt);

  void updateState(const Eigen::Quaterniond &base_q_WB,
                   const Eigen::Vector3d &base_vel_B,
                   const Eigen::Vector3d &base_angvel_B,
                   const Eigen::Vector3d &q, const Eigen::Vector3d &qd,
                   const Eigen::Vector3d &effort,
                   const Eigen::Vector3d &tool_pos_B,
                   const Eigen::Vector3d &tool_vel_B, const double &dt);

  void updateReference(const Eigen::Vector3d &pos_B,
                       const Eigen::Vector3d &vel_B,
                       const Eigen::Vector3d &acc_B,
                       const Eigen::Vector3d &force_B);

  void getControlCommand(Eigen::VectorXd *delta_pos_cmd,
  ...
  bool projectToWorkspace(Eigen::Vector3d *projection,
  ...
  void setBaseWrenchFilter(const bool set_filter, const double &cutoff_freq,
                           const double &dt);
...
protected:
  ...
  void getWorkspaceRegion(int *upper_index, int *lower_index,
                          const Eigen::Vector3d &point);

  bool projectToSphere(Eigen::Vector3d *projection,
                       const Eigen::Vector3d &point,
                       const Eigen::Vector3d &center, const double &radius,
                       const int direction);

  bool projectToHeight(Eigen::Vector3d *projection,
                       const Eigen::Vector3d &point, const double &height,
                       const int direction);
