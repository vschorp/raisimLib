...
home_path = task_path + "/../../../../.."
cfg_path = os.path.join(task_path, "config")

# config
config_fpath = os.path.join(cfg_path, args.config)
...
for update in range(1000000):
    ...
    if update % cfg["environment"]["eval_every_n"] == 0:
        ...
        if not on_cluster:
            ...
            env.start_video_recording(
                datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S") + "policy_" + str(update) + ".mp4"
            )
        ...
        for step in range(n_steps * 2):
            with torch.no_grad():
                ...
                if wait_time > 0.0 and not on_cluster:
    ...
    print(
        "{:<40} {:>6}".format(
            "real time factor: ", "{:6.0f}".format(total_steps / (end - start) * cfg["environment"]["control_dt"])
        )
    )
    print("----------------------------------------------------\n")
