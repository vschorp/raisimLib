...
void accelerometer::update() {
  ...
  measureGT_ = robot_orientation_rotmat.e().transpose() *
               ((imu_linear_velocity_W.e() - prev_imu_linear_velocity_W_.e()) /
                    sampleTime_ -
                gravity_);
...
void gyroscope::update() {
  ...
  measureGT_ =
      robot_orientation_rotmat.e().transpose() * robot_angular_velocity_W.e();
...
imuNoise::imuNoise(double sampleTime, imu_param parameters)
    : noise(sampleTime), params_(parameters) {
  for (int i = 0; i < 3; ++i) {
    turnOnBias_[i] = params_.turnOnBias_sigma *
...
odometryNoise::odometryNoise(double sampleTime, const Yaml::Node &cfg)
    : noise(sampleTime) {
...
Eigen::VectorXd odometryNoise::getNoise() {
  ...
  noise << pos_noise, orient_noise_quat.w(), orient_noise_quat.x(),
      orient_noise_quat.y(), orient_noise_quat.z(), lin_vel_noise,
      ang_vel_noise;
  return noise;
}
...
void odometry::update() {
  raisim::Vec<3> point_W, velocity_W, velocity_B, angularVelocity_W,
      angularVelocity_B;
  ...
  if (noiseSource_ != NULL) {
    ...
    Eigen::Quaterniond orient_W_quat(orient_W[0], orient_W[1], orient_W[2],
                                     orient_W[3]);
    Eigen::Quaterniond orient_noise_quat(noise[3], noise[4], noise[5],
                                         noise[6]);
    ...
    Eigen::Vector3d angularVelocity_B_noisy =
        angularVelocity_B.e() + noise.segment(10, 3);
...
void force::update() {
  ...
  // converting the tool force from world frame to the body frame. Since the
  // sensor measures it in the body frame
  raisim::Mat<3, 3> robot_orientation_rotmat;
  ...
  for (auto &contact : robot_->getContacts()) {
    if (contact.skip())
      continue;
    in_contact = true;
    contactMatProp_ = world_->getMaterialPairProperties(
        contact.getCollisionBodyA()->material,
        contact.getCollisionBodyB()->material);
    if (forceTipIdx == contact.getlocalBodyIndex()) {
      Eigen::Matrix3d rotMat_B = robot_orientation_rotmat.e().transpose() *
                                 contact.getContactFrame().e().transpose();
      ...
      // Compute the torque from the force measurement. This is equivalent to
      // (pos_B).cross(rotMat_B*contact.getImpulse().e()).
      contact_torque_impulse_B +=
          Skew(pos_B) * rotMat_B * contact.getImpulse().e();
    }
  }

  Eigen::Vector3d tool_force_B =
      impulseFilter_.update(contact_impulse_B) / sampleTime_;
  Eigen::Vector3d interaction_torque_B =
      angularImpulseFilter_.update(contact_torque_impulse_B) / sampleTime_;
...
void contact::update() {
  ...
  Eigen::Vector3d tool_force =
      impulseFilter_.update(contact_impulse_W) / sampleTime_;
  ; // world frame
