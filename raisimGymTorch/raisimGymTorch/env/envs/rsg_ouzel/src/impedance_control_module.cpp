...
//      updateVirtualMass(); // no virtual mass adaptation. no range finder
      ...
      Eigen::Vector3d ext_linear_accel_ =
              1.0 / mass_ * wrench_ext.thrust;
      Eigen::Vector3d ext_angular_accel_ =
              inertia_.inverse() * wrench_ext.torque;
      ...
      linear_accel_B_des +=
              R_W_B.transpose() * gravity_ * Eigen::Vector3d::UnitZ();

      // Translate control center from CoM to center of geometry. (add torque
      // due to COM offset)
      Eigen::Vector3d moment_ff =
              mass_ *
      ...
      static lowpassWrench commandFilter(linear_accel_B_des, angular_accel_B_des);
...
//      std::cout << cfg["use_force_control"].template As<bool>() << std::endl;
      ...
      control_params_.position_gain = cfg["lin_p_gain"].template As<float>() * linear_axis_gains;
      control_params_.velocity_gain = cfg["lin_d_gain"].template As<float>() * linear_axis_gains;
      control_params_.position_i_gain = cfg["lin_i_gain"].template As<float>() * linear_axis_gains;

      // rotational controller gains
      Eigen::Vector3d angular_axis_gains(cfg["roll_gain"].template As<float>(), cfg["pitch_gain"].template As<float>(),
                                         cfg["yaw_gain"].template As<float>());
      control_params_.attitude_gain = cfg["ang_p_gain"].template As<float>() * angular_axis_gains;
      control_params_.angular_rate_gain = cfg["ang_d_gain"].template As<float>() * angular_axis_gains;
      control_params_.attitude_i_gain = cfg["ang_i_gain"].template As<float>() * angular_axis_gains;

      control_params_.max_linear_accel = cfg["max_linear_accel"].template As<float>();
      control_params_.max_angular_accel = cfg["max_angular_accel"].template As<float>();

      control_params_.position_integrator_limit = cfg["position_integrator_limit"].template As<float>();
      control_params_.attitude_integrator_limit = cfg["attitude_integrator_limit"].template As<float>();
      ...
      if (cfg["base_virtual_inertia"].template As<float>() <= 0.0) {
        ...
        control_params_.virtual_inertia_inv =
                (1.0 / cfg["base_virtual_inertia"].template As<float>()) * Eigen::Vector3d::Ones();
      }
      ...
      control_params_.virtual_mass_decay = cfg["virtual_mass_decay"].template As<float>();

      // Force controller params
      control_params_.force_integrator_limit = cfg["force_integrator_limit"].template As<float>();
      ...
      control_params_.use_force_control = cfg["use_force_control"].template As<bool>();
...
//                      control_params_.range_d_max);
    ...
    void ImpedanceControlModule::computeCoriolisTerm(Eigen::Vector3d* n_coriolis) {
      ...
      *n_coriolis = inertia_.inverse() *
                    (ang_vel_B).cross(inertia_ * ang_vel_B);
    }
    ...
    Eigen::Vector3d ImpedanceControlModule::projectVector(
            const Eigen::Vector3d& vector_to_project,
